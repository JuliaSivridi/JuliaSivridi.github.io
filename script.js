const translations = {
  en: {
    ...
    proj_money_desc: 'Personal finance tracker available as a PWA and a native Android app. Both share the same Google Spreadsheet, so accounts, categories, and transactions stay in sync between web and phone. Tracks expenses, income, transfers, and debts across multiple accounts with multi-currency support. Full offline mode via IndexedDB (PWA) and Room (Android) — Google Sheets syncs in the background. Analytics with category donut charts, yearly income/expense overview, and a 12-month trailing bar chart.',
    proj_tasks_desc: 'Task manager available as a PWA and a native Android app. Both share the same Google Spreadsheet, so tasks, folders, and labels stay in sync between browser and phone. Supports subtasks at any depth, recurring tasks (deadline advances automatically on completion), priority levels (Urgent / Important / Normal), and Google Calendar integration — events appear inline with tasks in every view. Full offline mode via IndexedDB (PWA) and Room (Android). Android version adds four home screen widgets and swipe gestures for quick completion and rescheduling.',
    ...
    group_productivity: 'Productivity & Trackers',
    group_games: 'Games & Entertainment',
    group_feeds: 'Feeds & Bots',
    group_kids: 'Kids',
    label_github: 'GitHub',
    ...
    proj_money_desc: 'Henkilökohtainen taloudenhallintasovellus PWA:na ja natiivina Android-sovelluksena. Molemmat jakavat saman Google-taulukon, joten tilit, kategoriat ja tapahtumat pysyvät synkronoituina web- ja puhelinkäytön välillä. Seuraa menoja, tuloja, siirtoja ja velkoja useilla tileillä monikäyttöisellä valuuttatuella. Täydellinen offline-tila IndexedDB:n (PWA) ja Roomin (Android) kautta — Google Sheets synkronoidaan taustalla. Analytiikka kategoriadonitsikaaviolla, vuosittaisella tulo/meno-yhteenvedolla ja 12 kuukauden juoksevalla pylväskaaviolla.',
    proj_tasks_desc: 'Tehtävienhallintasovellus PWA:na ja natiivina Android-sovelluksena. Molemmat jakavat saman Google-taulukon, joten tehtävät, kansiot ja tunnisteet pysyvät synkronoituina selaimen ja puhelimen välillä. Tukee alitehtäviä rajattomalla syvyydellä, toistuvia tehtäviä (määräaika siirtyy automaattisesti suoritettaessa), prioriteettitasoja (Kiireellinen / Tärkeä / Normaali) ja Google Calendar -integraatiota — tapahtumat näkyvät tehtävien rinnalla jokaisessa näkymässä. Täydellinen offline-tila IndexedDB:n (PWA) ja Roomin (Android) kautta. Android-versio lisää neljä kotinäyttöwidgettiä ja pyyhkäisyeleitä nopeaan suoritukseen ja aikatauluttamiseen.',
    ...
    group_productivity: 'Tuottavuus & seurannat',
    group_games: 'Pelit & viihde',
    group_feeds: 'Syötteet & botit',
    group_kids: 'Lapsille',
    label_github: 'GitHub',
    label_github_web: 'GitHub Web',
    label_github_android: 'GitHub Android',
